...
| End-to-end pipeline | One integration test wires a `FakeRna` (already provided by RNA, `rna/fake.py`) into a real `ContextManager`, exercising the full `resolve()` pipeline without a real repository, real language tools, or real subprocesses — the same "fast, deterministic, safe for CI" posture `FakeRna` already gives every other host application in this codebase |

`FakeContextManager`/`FakeConversationManager` are scripted and deterministic (no SQLite, no threads, no RNA dependency), exactly the role `FakeRna` plays for RNA (`rna/docs/05_tool_contract_and_safety.md` §5) and the role `orchestrator/fake.py` already plays for `OrchestratorPort` elsewhere in this codebase — this subsystem is not introducing a new testing pattern, it is applying the one the codebase already uses everywhere a `Protocol`-based port exists.
